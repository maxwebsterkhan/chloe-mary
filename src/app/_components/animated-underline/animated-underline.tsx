...
const svgVariants = [
  ...
  `<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.99805 20.9998C65.6267 17.4649 126.268 13.845 187.208 12.8887C226.483 12.2723 265.751 13.2796 304.998 13.9998" stroke="currentColor" stroke-width="6" stroke-linecap="round"/></svg>`,
...
function decorateSVG(svgEl: SVGElement, svgClass: string) {
  svgEl.setAttribute("class", svgClass);
...
export default function AnimatedUnderline({
  ...
  const [variantIndex, setVariantIndex] = useState(() =>
    Math.floor(Math.random() * svgVariants.length)
  );
  ...
  useEffect(() => {
    if (!hovered) return;
    setVariantIndex((prev) => (prev + 1) % svgVariants.length);
  }, [hovered]);

  // Animate SVG underline on hover in
  useEffect(() => {
    const box = boxRef.current;
    if (!hovered || !box) return;

    box.innerHTML = svgVariants[variantIndex];
    const svg = box.querySelector("svg");
    if (svg) {
      decorateSVG(svg, styles.textDrawBoxSvg);
  ...
  const componentClass = `${styles.textDraw} ${className}`.trim();
  ...
  if (useNextLink) {
    return (
      <Link
        ...
        {...props}
      >
        <span className={styles.textDrawSpan}>{children}</span>
        <div ref={boxRef} className={styles.textDrawBox}></div>
      </Link>
    );
  }
